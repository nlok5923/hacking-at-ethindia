...
export function getAccountDeployer (entryPoint: string, owner: string): BytesLike {
  ...
  let provider = new ethers.providers.Web3Provider(ethereum);
  const accountCtr = new SimpleAccount__factory(provider.getSigner()).getDeployTransaction(entryPoint, owner).data!
  const factory = new Create2Factory(provider)
...
export async function getAggregatedAccountDeployer (entryPoint: string, aggregator: string): Promise<BytesLike> {
  ...
  let provider = new ethers.providers.Web3Provider(ethereum);
  const accountCtr = await new TestAggregatedAccount__factory(await provider.getSigner()).getDeployTransaction(entryPoint, aggregator).data!
